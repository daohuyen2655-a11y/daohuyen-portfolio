...
new_zodiac = f"""{{/* ─── 07. ZODIAC ─── */}}
        <section id="zodiac" className="relative w-full py-32 bg-[#F9F7F3] overflow-hidden">
           ...
           <div className="max-w-[1400px] mx-auto relative z-20 flex flex-col items-center">
              ...
              {{/* INTERACTIVE PAPER STACK - REVERTED TO LARGE OUTER SIZE */}}
                          ...
                          >
                              ...
                              <div className="w-full flex justify-between items-center mb-8 border-b border-[#EBE6DD] pb-6 shrink-0">
                                  ...
                                  <button 
                                      ...
                                      className="group flex items-center gap-2 font-sans font-light tracking-widest text-[#8C8377] hover:text-[#2C2822] transition-colors cursor-pointer text-sm md:text-base px-4 py-2 uppercase"
                                  >
                                      NEXT <span className="group-hover:translate-x-1 transition-transform">➔</span>
                                  </button>
                              </div>
                              
                              {{/* Invisible Scrollbar Content - CENTERED AND SCALED DOWN IMAGES */}}
                              <div className="flex-1 overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] w-full px-4 flex justify-center">
                                  <div className="w-full max-w-4xl columns-2 md:columns-3 lg:columns-4 gap-8 pb-10">
                                      {{page.images.map((img, i) => (
                                          <div key={{i}} className="relative rounded-[8px] overflow-hidden border border-[#F2EFEA] shadow-[0_4px_15px_rgba(0,0,0,0.04)] mb-8 break-inside-avoid bg-white">
