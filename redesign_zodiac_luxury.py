...
# We still have the js_array_str logic
import unicodedata
...
new_zodiac = f"""{{/* ─── 07. ZODIAC ─── */}}
        <section id="zodiac" className="relative w-full py-32 bg-gradient-to-b from-[#F2EFEA] to-[#D9D4C7] overflow-hidden">
           ...
           <div className="max-w-[1400px] mx-auto relative z-20 flex flex-col items-center">
              ...
              <div className="flex flex-col items-center mb-16 text-center px-6">
                  <div className="bg-[#2C3E50] px-8 py-2 rounded-full font-bold text-[#F2EFEA] shadow-xl text-sm md:text-base mb-6 uppercase font-['Quicksand'] tracking-[0.3em]">
                      07. Zodiac Collections
                  </div>
                  
                  {{/* Premium Scrapbook Typography */}}
                  <h2 className="font-['Fredoka'] text-[70px] md:text-[110px] text-transparent bg-clip-text bg-gradient-to-r from-[#2C3E50] to-[#4CA1AF] leading-none mb-8 text-center" 
                      style={{{{ filter: 'drop-shadow(3px 3px 0px rgba(255,255,255,1)) drop-shadow(8px 8px 15px rgba(44,62,80,0.2))' }}}}>
                      ZODIAC
                  </h2>
                  
                  <p className="max-w-2xl text-base md:text-lg text-[#546E7A] font-['Quicksand'] font-semibold leading-relaxed px-4">
              ...
              {{/* INTERACTIVE PAPER STACK - LUXURY DARK BOARDS */}}
              <div className="relative w-full max-w-[1100px] h-[650px] md:h-[800px] mx-auto mt-4 perspective-[1200px]">
                  {{{js_array_str}.map((page, idx) => {{
                      ...
                      return (
                          <div 
                              key={{idx}}
                              className="absolute inset-0 bg-gradient-to-br from-[#2C3E50] to-[#1A252F] rounded-[32px] border border-[#34495E] p-8 md:p-12 transition-all duration-[800ms] ease-[cubic-bezier(0.25,1,0.5,1)] overflow-hidden flex flex-col shadow-[0_25px_50px_rgba(0,0,0,0.3)]"
                              style={{{{
                          ...
                          >
                              ...
                              <div className="w-full flex justify-between items-center mb-8 border-b border-white/10 pb-6 shrink-0">
                                  <h3 className="font-['Fredoka'] font-semibold text-3xl md:text-5xl text-white tracking-wider drop-shadow-md">
                                      {{page.title}}
                                  </h3>
                                  <button 
                                      onClick={{() => setActiveZodiacPage(prev => (prev + 1) % 5)}}
                                      className="group flex items-center gap-3 font-['Quicksand'] font-bold text-[#2C3E50] bg-[#F2EFEA] hover:bg-white transition-colors cursor-pointer text-sm md:text-base px-6 py-3 rounded-full shadow-lg"
                                  >
                              ...
                              <div className="flex-1 overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] w-full px-2">
                                  <div className="columns-1 sm:columns-2 md:columns-3 gap-8 pb-10">
                                      {{page.images.map((img, i) => (
                                          <div key={{i}} className="relative rounded-[16px] overflow-hidden border-[4px] border-[#F2EFEA] shadow-[0_10px_30px_rgba(0,0,0,0.4)] mb-8 break-inside-avoid bg-white group cursor-pointer hover:shadow-[0_20px_40px_rgba(0,0,0,0.5)] transition-shadow duration-500 hover:-translate-y-1">
